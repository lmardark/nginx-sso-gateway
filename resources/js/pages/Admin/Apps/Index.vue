                ...
                <div class="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-[#706f6c] dark:text-[#A1A09A]">
                            {{ apps.length }} {{ apps.length === 1 ? 'aplicação cadastrada' : 'aplicações cadastradas' }}
                        </p>
                    </div>
                    <a
                ...
                <div class="overflow-hidden rounded-lg shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d]">
                    ...
                    >
                        ...
                        <p class="text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC]">
                            {{ search ? 'Nenhuma aplicação encontrada' : 'Nenhuma aplicação cadastrada' }}
                        </p>
                        <p class="mt-1 text-xs text-[#706f6c] dark:text-[#A1A09A]">
                            {{ search ? 'Tente outro termo de busca.' : 'Clique em "Nova aplicação" para começar.' }}
                        </p>
                    </div>
                    ...
                    <table v-else class="w-full bg-white text-sm dark:bg-[#161615]">
                        <thead>
                            <tr class="border-b border-[#e3e3e0] dark:border-[#3E3E3A]">
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#706f6c] dark:text-[#A1A09A]">Nome</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#706f6c] dark:text-[#A1A09A]">Domínios</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#706f6c] dark:text-[#A1A09A]">API Key</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#706f6c] dark:text-[#A1A09A]">Status</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#706f6c] dark:text-[#A1A09A]">Criado em</th>
                                <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wide text-[#706f6c] dark:text-[#A1A09A]">Ações</th>
                            </tr>
                        </thead>
                            ...
                            >
                                ...
                                <td class="px-6 py-4 text-[#706f6c] dark:text-[#A1A09A]">
                                    <div class="flex flex-wrap gap-1">
                                        ...
                                        <span v-if="app.allowed_domains.length === 0" class="text-xs italic">
                                            Nenhum
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <code class="max-w-[140px] truncate rounded bg-[#f5f5f3] px-2 py-0.5 font-mono text-xs text-[#1b1b18] dark:bg-[#1e1e1c] dark:text-[#EDEDEC]">
                                            {{ app.api_key.substring(0, 16) }}…
                                        </code>
                                        <button
                                            type="button"
                                            class="text-[#706f6c] transition hover:text-[#1b1b18] dark:text-[#A1A09A] dark:hover:text-[#EDEDEC]"
                                            :title="copiedKey === app.id ? 'Copiado!' : 'Copiar API Key'"
                                            @click="copyKey(app.id, app.api_key)"
                                        >
                                            <svg v-if="copiedKey !== app.id" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        >
            <div class="w-full max-w-sm rounded-lg bg-white shadow-xl dark:bg-[#161615]">
                <div class="p-6">
                    ...
                    <h3 class="text-base font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">Excluir aplicação</h3>
                    <p class="mt-1 text-sm text-[#706f6c] dark:text-[#A1A09A]">
